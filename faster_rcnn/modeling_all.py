...
def train(bert_model,bfmc_model,options,im_embeddings_dir,optimizer,scheduler,dataloader):
    ...
    for batch_idx,batch in enumerate(dataloader):
        ...
        idx_count=1
        for sub_count in range(7):
            pickup_indices=list()
            if sub_count==6:
                pickup_indices.append(0)
                pickup_indices.append(19)
            else:
                pickup_indices.append(0)
                pickup_indices.append(idx_count)
                pickup_indices.append(idx_count+1)
                pickup_indices.append(idx_count+2)
            idx_count+=3

            sub_inputs_embeds=torch.empty(batch_size,len(pickup_indices),512,768,dtype=torch.float).to(device)
            sub_attention_mask=torch.empty(batch_size,len(pickup_indices),512,dtype=torch.long).to(device)
            sub_token_type_ids=torch.empty(batch_size,len(pickup_indices),512,dtype=torch.long).to(device)

            for i in range(batch_size):
                for j,pickup_index in enumerate(pickup_indices):
                    sub_inputs_embeds[i,j]=bfmc_inputs["inputs_embeds"][i,pickup_index]
                    sub_attention_mask[i,j]=bfmc_inputs["attention_mask"][i,pickup_index]
                    sub_token_type_ids[i,j]=bfmc_inputs["token_type_ids"][i,pickup_index]
