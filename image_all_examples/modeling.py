...
def train(bert_model,bfmc_model,options,im_embeddings_dir,optimizer,scheduler,dataloader):
    ...
    for batch_idx,batch in enumerate(dataloader):
        ...
        for i in range(7):
            used_option_indices=[]
            used_option_indices.append(0)

            if i==6:
                used_option_indices.append(19)
            else:
                used_option_indices.append(base_index)
                used_option_indices.append(base_index+1)
                used_option_indices.append(base_index+2)
                base_index+=3

            sub_inputs_embeds=torch.empty(batch_size,len(used_option_indices),512,768,dtype=torch.long).to(device)
            sub_attention_mask=torch.empty(batch_size,len(used_option_indices),512,dtype=torch.long).to(device)
            sub_token_type_ids=torch.empty(batch_size,len(used_option_indices),512,dtype=torch.long).to(device)

            for j in range(batch_size):
                for k,used_option_index in enumerate(used_option_indices):
                    sub_inputs_embeds[j,k]=bfmc_inputs["inputs_embeds"][j,used_option_index]
                    sub_attention_mask[j,k]=bfmc_inputs["attention_mask"][j,used_option_index]
                    sub_token_type_ids[j,k]=bfmc_inputs["token_type_ids"][j,used_option_index]

            sub_bfmc_inputs={
                "inputs_embeds":sub_inputs_embeds,
                "attention_mask":sub_attention_mask,
                "token_type_ids":sub_token_type_ids,
                "labels":bfmc_inputs["labels"]
            }

            # Initialize gradiants
            optimizer.zero_grad()
            # Forward propagation
            outputs = bfmc_model(**sub_bfmc_inputs)
            loss = outputs[0]
...
def main(batch_size,num_epochs,lr,train_input_dir,dev1_input_dir,im_embeddings_dir,result_save_dir):
    ...
    total_steps*=7  #Use all examples
...
if __name__=="__main__":
    ...
    parser.add_argument("--result_save_dir",type=str,default="./OutputDir/ImageBERTAllExamples")
