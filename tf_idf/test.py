...
def calc_score(mecab,question,option,count_dir,nqis):
    genkeis=[]
    ...
    for genkei in genkeis:
        ...
        tf=freq/d
        idf=math.log(NUM_TOTAL_DOCS/nqi)+1.0
...
def evaluate(classifier_model,dataloader,examples,mecab,count_dir,nqis):
    ...
    for batch_idx,batch in tqdm(enumerate(dataloader),total=len(dataloader)):
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                scores=np.empty(20)
                for j in range(20):
                    score=calc_score(mecab,example.question,example.endings[j],count_dir,nqis)
                    scores[j]=score
...
def main(
    ...
    nqis_filepath,test_upper_bound,result_save_dir):
